...
def parse_li(li):
    ...
    title = re.search('.+?<strong>(?P<title>[^</strong>]*).+?', li).group("title")
    ...
    job["jobTitle"] = title
    job["jobHref"] = href
    job["jobPlan"] = attr_value.get("招聘计划") if "招聘计划" in attr_value.keys() else None
    job["jobWay"] = attr_value.get("报名方式") if "报名方式" in attr_value.keys() else None
    job["jobDate"] = attr_value.get("报名时间") if "报名时间" in attr_value.keys() else None
    job["jobLocation"] = attr_value.get("招聘地区") if "招聘地区" in attr_value.keys() else None
    job["jobType"] = attr_value.get("劳动关系") if "劳动关系" in attr_value.keys() else None
